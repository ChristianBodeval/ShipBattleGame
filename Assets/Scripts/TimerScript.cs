...
{
    public  int         roundLenghtS;                                               // The amount of time in seconds that a round lasts
    ...
    {
        roundLenghtS = 240;                                                         // The length of the round (in seconds) is defined
        timerUI = GetComponent<TMP_Text>();                                         // The Timer_Text component is asigned to the timerUI variable
        countDownTimer = roundLenghtS;                                              // The start value for the countDownTimer is asigned to be the lenght of the round
    ...
    void FixedUpdate()                                                              // Update is called once per frame
    {
        ...
        if (countDownTimer > 0)                                                     // an if statment is used to stop the countdown when zero(0) is reached
            countDownTimer = (int) (roundLenghtS - (Time.time - timeElapced));      // The time left is calculatet by subtracting the amount of time passed from when the timer UI was called, from the total amount of time sins the game was started and then subtracting that from the lenght of the round in seconds
